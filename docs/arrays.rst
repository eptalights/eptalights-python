...
Let’s examine the first instruction in the function ``main`` from the file ``07_array.cc``. The instruction is an assignment (``OpType.ASSIGN``), represented internally by ``SophiaIRAssignModel``, and assigns the constant ``10`` to the first element of the array ``arr``.
