...
from eptalights.models.sophia_ir.tokenized_operand import TokenizedOperandModel
from eptalights.models.sophia_ir.cfg import ControlFlowGraphModel
from eptalights.models.sophia_ir.callsite import CallsiteManagerModel
from eptalights.models.sophia_ir.variable import VariableManagerModel
...
class SophiaIRGotoModel(SophiaIRBaseModel):
    ...
    dst : TokenizedOperandModel, optional
        The destination operand for the `goto` statement. Defaults to `None`.
    goto_label_names : List[str], optional
        A list of possible label names the `goto` statement may jump to.
        Defaults to an empty list.
    goto_basic_blocks : List[int], optional
        A list of possible basic block indices the `goto` statement may target.
        Defaults to an empty list.

    Notes
    -----
    - If the `goto` statement targets a variable (e.g., `goto varname`), it may
      resolve to multiple labels, similar to a `switch` statement.
    - If it targets a constant (e.g., `goto constant`), it resolves to a single
      static label.
    """
...
class FunctionModel(BaseModel):
    ...
    class_name : Optional[str], optional
        ...
        Defaults to `None`.
    class_props : Dict[str, TokenizedOperandModel], optional
        A dictionary mapping class property names to their tokenized representations.
